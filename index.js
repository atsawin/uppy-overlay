...
class UppyOverlay extends Plugin {
  constructor (uppy, opts) {
    super(uppy, opts)
    this.id = this.opts.id || 'Overlay'
    this.type = 'modifier'

    this.defaultLocale = {
      strings: {
        overlayingImages: 'Overlaying images...'
      }
    }
  ...
  prepareUpload (fileIDs) {
    const promises = fileIDs.map((fileID) => {
      ...
      this.uppy.emit('preprocess-progress', file, {
        mode: 'indeterminate',
        message: this.i18n('overlayingImages')
      })
